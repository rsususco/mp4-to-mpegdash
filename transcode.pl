...
my $versions = [ '320', '640', '720', '1280', '1920', ];

sub create_multiple_bitrate_versions {
	my ($filename) = @_;
	my $lastVersion = '';
	for my $_version (@{$versions}){
		my $r = `ffmpeg -threads 16 -i $filename -preset ultrafast -vf scale=$_version:-1:force_original_aspect_ratio=decrease -x264opts 'keyint=$config->{keyint}:min-keyint=$config->{keyint}:no-scenecut' -strict -2 -r $config->{framerate} $_version/$filename -y`;
